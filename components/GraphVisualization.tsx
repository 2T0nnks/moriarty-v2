...
export default function GraphVisualization({
    ...
    return (
        <div id={id}>
            {title && <p className="text-xs mb-2 font-medium" style={{ color: 'var(--text-3)' }}>{title}</p>}
            <svg width={width} height={height} className="mx-auto" viewBox={`0 0 ${width} ${height}`}>
                ...
                {edges.map(({ i, j, cut }) => (
                    <line
                        ...
                        stroke={cut ? 'var(--accent-yellow)' : 'var(--border-1)'}
            ...
            {/* Legend */}
            <div className="flex items-center justify-center gap-4 mt-2 text-[10px]" style={{ color: 'var(--text-3)' }}>
                {solutionBitstring && (<>
